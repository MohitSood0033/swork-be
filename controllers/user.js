...
const getServiceDoc = async (req, res) => {
  try {
    ...
    console.log("getServiceDoc",getServiceDoc)
    // console.log(getUser.service)
    // let ServiceId=getUser.service
    // const service=await Service.findOne({_id: {$eq:ServiceId}})
    // const ServiceName=service.name
...
const addserviceUserBank = async (req, res) => {
  try {

    let token = req.headers["x-access-token"];
    let decoded = jwt.verify(token, process.env.TOKEN_KEY);
    const id = decoded.id;
    const serviceuser =await User.updateOne(
      { _id: { $eq:id } },
      {
        $set: {
    ...
    if(serviceuser){
      return res.status(200).json({message:"Bank details add Successfully",serviceuser})
    }else{
      res.status(400).json({
